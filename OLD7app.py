...
def load_and_process_data(selected_date):
    ...
    df = pd.read_excel("./data/Arrivals_August.xlsx")
...
def generate_plot(df, start_date_arr, end_date_arr):
    ...
    df_total = pd.read_excel("./data/Arrivals_August.xlsx")
    ...
    logging.info("Colors used for inner ring: %s", [lightened_color_map[cat] for cat in latest_day_percentages.index])

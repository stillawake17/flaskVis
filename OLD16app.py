...
def load_and_process_data(selected_date):
    ...
    df["Year"] = df["FullLandedTime"].dt.year
    df["Month"] = df["FullLandedTime"].dt.month_name()
    df["Week"] = df["FullLandedTime"].dt.strftime("%U")
...
def filter_data_by_time_period(df, time_period, period_value=None):
    """
    Filter the dataframe based on the time_period and period_value.
    
    :param df: Dataframe to be filtered
    :param time_period: 'monthly', 'weekly', or 'total'
    :param period_value: The specific month or week to filter on, if applicable
    :return: Filtered dataframe
    """
    if time_period == "monthly" and period_value:
        return df[(df["Month"] == period_value.split("-")[0]) & (df["Year"] == int(period_value.split("-")[1]))]
    elif time_period == "weekly" and period_value:
        return df[(df["Week"] == period_value.split("-")[0]) & (df["Year"] == int(period_value.split("-")[1]))]
    elif time_period == "total":
        return df
    else:
        raise ValueError("Invalid time_period or period_value.")
...
def generate_plot(df):
    img = io.BytesIO()  # Create a BytesIO object to hold the plot image data
    ...
    plt.savefig(img, format='png')
    img.seek(0)  # Move the cursor to the beginning of the stream
    
    # Encode the image in base64 and decode it to utf-8 to embed it in HTML
...
def index():
    ...
    # filter_type = request.args.get('filter_type', 'total')  # get filter_type from query parameters or default to 'total'
